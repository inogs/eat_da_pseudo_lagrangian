...
a=np.loadtxt("../spinup_folder_list.txt",dtype=np.dtype('U'))

for test_dir in get_iterable(a):
...
#filenames = glob.glob('/g100_scratch/userexternal/gocchipi/stoc_prova/*/result.nc')
    ...
    for inc,ncname in enumerate(get_iterable(filenames)):
        infile=ncname 
...
#       ax.set_xticklabels(tick_labels,rotation=90)
    ...
    floatname = test_dir.split('/')[5][:7]
    prmtr_nm  = test_dir.split('/')[5][8:][:-16]
    fileout='chl_' + floatname  + '_' + prmtr_nm  + 'F1_plot.png'
