...
for iir,file_res in enumerate(LISTresults):
    ...
    for pp in range(4):
        ppstr = 'P' + np.str(pp+1) + '_c'
        values = values + NCin.variables[ppstr][:,:,0,0].data
    for jj in range(nT):
        valinterp = np.interp(zinterp,NCin.variables['z'][jj,:,0,0].data,values[jj,:])
        P_c = np.nansum(valinterp[maskz]*deltaz)
        varMat[jj,iir] = P_c  #gC/m2/y

    NCin.close()


P_cmean = varMat.mean(axis=1)
P_cstd = varMat.std(axis=1)

LIST = []
LIST.append(P_cmean)
LIST.append(P_cstd)

outfile = OUTDIR + '/P_c200.npy'
np.save(outfile,LIST)

outfile = OUTDIR + '/P_c200.txt'
